    ...
    for fr = fr_vals
        for rs = rs_vals
            for rg = rg_vals
                ...
                %% KGE
                KGE(Ind,1)=1-sqrt((corr(Qsim,Qobs)-1)^2+(std(Qsim)/std(Qobs)-1)^2+(mean(Qsim)/mean(Qobs)-1)^2);
                %% NSE 
                NSE(Ind,1) = 1 - ( sum( (Qobs - Qsim).^2 ) / sum( (Qobs - mean(Qobs)).^2 ) ); 
                %% Saving parameters
...
[bestNSE, bestNSEind] = max(NSE);
bestParNSE = Par(bestNSEind, :)
